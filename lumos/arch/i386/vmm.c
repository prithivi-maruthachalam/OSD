...
void init_vmm(){
    ...
    pde_set_attrib(entry2, I86_PDE_WRITABLE);
    pde_set_addr(entry2, (physical_addr_t)table2);

    // store current page dir
    _cur_directory = dir;

    vmm_switch_page_dir(dir);
...
void vmm_map_page(void *phys_addr, void* virt_addr){
    ...
    // get the page
    pte_t *page = vmm_ptable_lookup_entry(virt_addr);
